...
def run_vcl_ibp(hidden_size, no_epochs, data_gen, batch_size=None, single_head=True):
    in_dim, out_dim = data_gen.get_dims()
    x_testsets, y_testsets = [], []
    ...
    for task_id in range(data_gen.max_iter):
        ...
        # Train on non-coreset data
        mf_model = MFVI_IBP_NN(in_dim, hidden_size, out_dim, x_train.shape[0], prev_means=mf_weights, prev_log_variances=mf_variances)
        mf_model.train(x_train, y_train, head, no_epochs, bsize)
        mf_weights, mf_variances = mf_model.get_weights()
